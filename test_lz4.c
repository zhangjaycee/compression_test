...
int main()
{
    ...
    for(int i = 1; i <= 64; i++) {
        ...
        char *buf = (char *)malloc(rd_size*2+5);
        char *decom_buf = (char *)malloc(rd_size*2+5);
        char *dst_buf = (char *)malloc(rd_size*2+5);
        char *dst_buf_hc = (char *)malloc(rd_size*2+5);
        
        /*
        read(fd, buf, rd_size);
        char *print_buf = (char *)malloc(35);
        memcpy(print_buf, buf, 30);
        print_buf[30] = '\0';
        printf("%s\n",print_buf);
        free(print_buf);
        */
    ...
    /*
        printf("%d\t%0.1f\t%0.4f\t%0.4f\t%0.2f\t%0.4f\t%0.2f\t%0.1f\t%0.4f\t%0.4f\t%0.2f\t%0.4f\t%0.2f\n", 
                            src_size, (float)dst_size/REC_TIMES, cr, seconds_avg,MS_1000/(seconds_avg/i*2)/1024, seconds_avg_decom, MS_1000/(seconds_avg_decom/i*2)/1024, 	
                                      (float)dst_size_hc/REC_TIMES, cr_hc, seconds_avg_hc, MS_1000/(seconds_avg_hc/i*2)/1024, seconds_avg_hc_decom,  MS_1000/(seconds_avg_hc_decom/i*2)/1024);
    */
        free(buf);
